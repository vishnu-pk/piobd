...
class OBD_Recorder():
    def __init__(self, path, log_items):
        ...
        filename = path+"car-"+str(localtime[0])+"-"+str(localtime[1])+"-"+str(localtime[2])+"-"+str(localtime[3])+"-"+str(localtime[4])+"-"+str(localtime[5])+".csv"
        self.log_file = open(filename, "w", 128)
        self.log_file.write("Time,RPM,Throttle,Temperature,STFT1,MAP,TimingAdvance,IntakeTemp,O2\n");
...
logitems = ["rpm", "throttle_pos", "temp", "short_term_fuel_trim_1","manifold_pressure", "timing_advance", "intake_air_temp", "o211" ]
o = OBD_Recorder('/home/'+username+'/log/01Dec18/', logitems)
o.connect()
